...
''');
  ...
  Future<bool> hasOverlappingClasses(
    DateTime startTime,
    double duration,
    ...
    // Query for any classes that overlap with the given time range
    final result = await db.rawQuery(
      '''
      SELECT * FROM $tableClasses
      WHERE 
        (${ClassFields.status} != ? AND ${ClassFields.status} != ?) AND
        (
          (datetime(${ClassFields.dateTime}) < datetime(?) AND 
           datetime(${ClassFields.dateTime}, '+' || (${ClassFields.duration} * 60) || ' minutes') > datetime(?))
          OR
          (datetime(${ClassFields.dateTime}) >= datetime(?) AND 
           datetime(${ClassFields.dateTime}) < datetime(?, '+' || ? || ' minutes'))
        )
        ${excludeClassId != null ? 'AND ${ClassFields.id} != ?' : ''}
    ''',
      [
        ClassStatus.cancelled.toString().split('.').last,
        ClassStatus.completed.toString().split('.').last,
        ...
        if (excludeClassId != null) excludeClassId,
      ],
    );
  ...
  Future<Class> addClass(Class classItem) async {
    ...
    final hasOverlap = await hasOverlappingClasses(
      classItem.dateTime,
      classItem.duration,
      null,
    );
    if (hasOverlap) {
      throw Exception('This time slot overlaps with another class');
    }
  ...
  Future<int> updateClass(Class classItem) async {
    ...
    // Check for overlapping classes, excluding the current class being updated
    final hasOverlap = await hasOverlappingClasses(
      classItem.dateTime,
      classItem.duration,
      classItem.id,
  ...
  Future<List<Class>> getPastClasses(
    String studentId,
    String subjectId,
    DateTime currentClassDate,
  ) async {
    final db = await instance.database;
    final result = await db.rawQuery(
      '''
      ...
      AND c.${ClassFields.subjectId} = ?
      AND c.${ClassFields.dateTime} < ?
      AND c.${ClassFields.status} = ?
      ORDER BY c.${ClassFields.dateTime} DESC
      LIMIT 10
    ''',
      [
        studentId,
        subjectId,
        currentClassDate.toIso8601String(),
        ClassStatus.completed.toString().split('.').last,
      ],
    );
